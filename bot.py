...
async def handle_document(update: Update, context: CallbackContext):
    ...
    # === PROXIES.TXT UPLOAD SUPPORT ===
    if filename_lower == "proxies.txt":
        try:
            file_info  = await context.bot.get_file(doc.file_id)
            downloaded = await file_info.download_as_bytearray()
            content    = downloaded.decode("utf-8", errors="ignore")

            # Save/overwrite proxies.txt in working directory
            with open("proxies.txt", "w", encoding="utf-8") as f:
                f.write(content)

            # Reload for current user
            us.proxy_manager.load_proxies()
            proxy_count = len(us.proxy_manager.proxies)

            if proxy_count > 0:
                # Apply a fresh proxy to current session
                us.proxy_manager.rotate_proxy(us.session)
                await update.message.reply_text(
                    "────────────────────────\n"
                    "  ✅ *ᴘʀᴏxɪᴇs ᴜᴘʟᴏᴀᴅᴇᴅ*\n"
                    "────────────────────────\n\n"
                    f"*◆ ʟᴏᴀᴅᴇᴅ:* `{proxy_count}` ᴘʀᴏxɪᴇs\n"
                    "*✦ ᴘʀᴏxʏ sᴜᴘᴘᴏʀᴛ ɪs ɴᴏᴡ ᴀᴄᴛɪᴠᴇ!*\n"
                    "*ᴛʜᴇ ʙᴏᴛ ᴡɪʟʟ ᴀᴜᴛᴏ-ʀᴏᴛᴀᴛᴇ ᴘʀᴏxɪᴇs ᴛᴏ ᴀᴠᴏɪᴅ ɪᴘ ʙʟᴏᴄᴋs.*\n\n"
                    "*ʏᴏᴜ ᴄᴀɴ ɴᴏᴡ sᴛᴀʀᴛ ʙᴜʟᴋ ᴄʜᴇᴄᴋs — ɪᴛ ᴡᴏɴ'ᴛ sᴛᴏᴘ ᴏɴ ɪᴘ ʙʟᴏᴄᴋs.*",
                    parse_mode="Markdown",
                    reply_markup=InlineKeyboardMarkup(BACK_MENU),
                )
            else:
                await update.message.reply_text(
                    "*⚠ ᴘʀᴏxɪᴇs.txt ᴜᴘʟᴏᴀᴅᴇᴅ ʙᴜᴛ ɴᴏ ᴠᴀʟɪᴅ ᴘʀᴏxɪᴇs ғᴏᴜɴᴅ.*\n"
                    "ᴍᴀᴋᴇ sᴜʀᴇ ᴇᴀᴄʜ ʟɪɴᴇ ɪs ᴀ ᴠᴀʟɪᴅ ᴘʀᴏxʏ (ɪᴘ:ᴘᴏʀᴛ ᴏʀ ʜᴛᴛᴘ://ɪᴘ:ᴘᴏʀᴛ)",
                    parse_mode="Markdown",
                )
            return
        except Exception as e:
            await update.message.reply_text(f"*✘ ᴇʀʀᴏʀ sᴀᴠɪɴɢ ᴘʀᴏxɪᴇs.txt:* `{str(e)[:100]}`",
                                             parse_mode="Markdown")
            return

    # === NORMAL COMBO .TXT (BULK CHECK) ===
    try:
        file_info  = await context.bot.get_file(doc.file_id)
        downloaded = await file_info.download_as_bytearray()
        content    = downloaded.decode("utf-8", errors="ignore")
