    ...
    try:
        ...
        if attempt == 1:
            ...
            try:
                ...
                if return_code != 0 or any(hint in combined_output for hint in ['Please install', 'pip install', 'ModuleNotFoundError']):
                    # Pattern 1: ModuleNotFoundError: No module named 'xxx'
                    match_py = re.search(r"ModuleNotFoundError: No module named '(.+?)'", combined_output)
                    # Pattern 2: pip install pkg1 pkg2 pkg3 (from script hint messages)
                    match_pip_hint = re.search(r'pip install\s+([\w\s\-]+)', combined_output)

                    if match_py:
                        module_name = match_py.group(1).strip().strip("'\"").split('.')[0]
                        ...
                        if attempt_install_pip(module_name, message_obj_for_reply):
                            ...
                            return
                    elif match_pip_hint and return_code != 0:
                        packages = match_pip_hint.group(1).strip().split()
                        logger.info(f"Detected pip install hint, packages: {packages}")
                        bot.reply_to(message_obj_for_reply, f"Script requires packages: `{' '.join(packages)}`\nInstalling...", parse_mode='Markdown')
                        all_ok = True
                        for pkg in packages:
                            pkg = pkg.strip()
                            if not pkg: continue
                            ...
                            if result_pip.returncode != 0:
                                ...
                                bot.reply_to(message_obj_for_reply, f"❌ Failed to install `{pkg}`.", parse_mode='Markdown')
                                all_ok = False
                            else:
                                logger.info(f"Installed {pkg} OK.")
                        if all_ok:
                            bot.reply_to(message_obj_for_reply, f"✅ All packages installed. Retrying '{file_name}'...")
                            time.sleep(2)
                            threading.Thread(target=run_script, args=(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt + 1)).start()
                            return
                        else:
                            bot.reply_to(message_obj_for_reply, f"Some packages failed to install. Cannot run '{file_name}'.")
                            return
                    elif return_code != 0:
