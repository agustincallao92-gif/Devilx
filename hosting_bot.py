...
def generate_key(plan: str, created_by: int) -> str | None:
    ...
    with DB_LOCK:
        ...
        try:
            c.execute(
                'INSERT INTO redeem_keys (key, plan, days, created_by, created_at) VALUES (?, ?, ?, ?, ?)',
                (key, plan, days, created_by, datetime.now().isoformat())
            )
            conn.commit()
            logger.info(f"Key {key} ({plan}) generated by {created_by}")
            return key
        except Exception as e:
...
def redeem_key_db(key: str, user_id: int):
    ...
    with DB_LOCK:
        ...
        c = conn.cursor()
        try:
            c.execute('SELECT plan, days, used_by FROM redeem_keys WHERE key = ?', (key,))
            row = c.fetchone()
            if not row:
                return False, "❌ Invalid key. Please check and try again."
            plan, days, used_by = row
            if used_by is not None:
                return False, "❌ This key has already been used."
            # Mark as used
            c.execute(
                'UPDATE redeem_keys SET used_by = ?, used_at = ? WHERE key = ?',
            ...
            return True, f"✅ Key redeemed! Plan: *{label}*\nExpiry: `{exp_str}`"
        except Exception as e:
...
def list_keys_db(created_by: int = None, show_used: bool = False):
    ...
    try:
        if created_by:
            c.execute('SELECT key, plan, days, used_by, created_at FROM redeem_keys WHERE created_by = ? ORDER BY created_at DESC LIMIT 20', (created_by,))
        else:
            c.execute('SELECT key, plan, days, used_by, created_at FROM redeem_keys ORDER BY created_at DESC LIMIT 20')
        return c.fetchall()
    finally:
        conn.close()
...
def gen_key_menu_callback(call):
    ...
    try:
        bot.edit_message_text(
            "🎫 *Generate Subscription Key*\n\nSelect a plan to generate a key for:",
            call.message.chat.id, call.message.message_id,
...
def handle_genkey_plan(call):
    ...
    if not plan_info:
        ...
        return
    bot.answer_callback_query(call.id, f"Generating {plan_info['label']} key...")
    key = generate_key(plan, user_id)
    if not key:
        bot.send_message(call.message.chat.id, "❌ Failed to generate key. Try again.")
        return
    text = (
        f"🎫 *New Subscription Key Generated!*\n\n"
        f"Plan: *{plan_info['label']}*\n"
        f"Key:\n`{key}`\n\n"
        f"Share this key with your customer.\n"
        f"They can redeem it with:\n`/redeem {key}`"
    )
    markup = types.InlineKeyboardMarkup()
    markup.row(types.InlineKeyboardButton('🎫 Generate Another', callback_data='gen_key_menu'))
