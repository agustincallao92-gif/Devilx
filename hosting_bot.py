import psutil
"""
Manjiro Bot Hosting - v2.0.4
full updated
all original has deleted and maked new one
"""
...
TOKEN = '8268389194:AAFQ__wVslIfwEerDQNRRKhwd8zVhqwoqmg'
OWNER_ID = 6854608129
ADMIN_ID = 6854608129
YOUR_USERNAME = '@devxyto'
UPDATE_CHANNEL = '1'

GEMINI_API_KEY = os.environ.get("AQ.Ab8RN6Jk85rARpjd5BoME8mUmdNI5piFU3xDFqNs1qL6vrF7Bw", "AQ.Ab8RN6Jk85rARpjd5BoME8mUmdNI5piFU3xDFqNs1qL6vrF7Bw")
...
bot_locked = False
ai_chat_users = set()  # Users currently in AI chat mode

# Tracks in-progress key generation state per admin: {user_id: {'plan': ..., 'qty': ..., 'max_uses': ...}}
pending_genkey = {}
...
COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    ...
    ["📞 Contact Owner", "🤖 MANJIRO AI"],
    ["💬 AI Chat", "🚪 Exit AI Chat"]
]
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        if attempt == 1:
            # Use a safe import-only check: compile the script and check imports
            # WITHOUT actually running the script (avoids token conflicts with Telegram)
            check_command = [sys.executable, '-c',
                f"import py_compile, sys; py_compile.compile({repr(script_path)}, doraise=True); "
                f"import ast, builtins; src=open({repr(script_path)}).read(); ast.parse(src)"]
            logger.info(f"Running Python syntax pre-check for: {script_path}")
            check_proc = None
            try:
                ...
                logger.info(f"Python syntax check. RC: {return_code}. Stderr: {stderr[:200]}...")

                if return_code != 0:
                    # Syntax error in the script
                    error_summary = combined_output[:500]
                    bot.reply_to(message_obj_for_reply, f"❌ Syntax error in '{file_name}':\n```\n{error_summary}\n```\nFix the script.", parse_mode='Markdown')
                    return

                # Scan the script source for import statements and pre-install missing packages
                missing_modules = []
                try:
                    ...
                    for mod in imported:
                        pkg = TELEGRAM_MODULES.get(mod.lower()) or TELEGRAM_MODULES.get(mod)
                        if pkg is None:
                            continue  # stdlib, skip
                        try:
                            __import__('importlib').import_module(mod)
                        except ImportError:
                            missing_modules.append((mod, pkg))
                except Exception as scan_err:
                    logger.warning(f"Could not scan imports for {file_name}: {scan_err}")

                if missing_modules:
                    pkg_list = ', '.join(f'`{pkg}`' for _, pkg in missing_modules)
                    bot.reply_to(message_obj_for_reply, f"📦 Installing missing packages: {pkg_list}...", parse_mode='Markdown')
                    all_ok = True
                    for mod, pkg in missing_modules:
                        ...
                        if result_pip.returncode != 0:
                            logger.error(f"Failed to install {pkg}: {result_pip.stderr}")
                            bot.reply_to(message_obj_for_reply, f"❌ Failed to install `{pkg}`. Cannot run '{file_name}'.", parse_mode='Markdown')
                            all_ok = False
                            break
                        else:
                            logger.info(f"Pre-installed {pkg} OK.")
                    if not all_ok:
                        return
                    bot.reply_to(message_obj_for_reply, f"✅ Packages installed. Starting '{file_name}'...")
...
KEY_PLANS = {
    ...
    '8d':       {'label': '8 Days',    'days': 8},
    '15d':      {'label': '15 Days',   'days': 15},
    '1month':   {'label': '1 Month',   'days': 30},
    '3months':  {'label': '3 Months',  'days': 90},
    'lifetime': {'label': 'Lifetime',  'days': 36500},
}

def generate_key(plan: str, created_by: int, qty: int = 1, max_uses: int = 1) -> list[str]:
    ...
    with DB_LOCK:
        ...
        try:
            for _ in range(qty):
                ...
                key = f"MJR-{raw[:5]}-{raw[5:10]}-{raw[10:15]}-{raw[15:20]}"
                c.execute(
                    'INSERT INTO redeem_keys (key, plan, days, created_by, created_at, max_uses, use_count) VALUES (?, ?, ?, ?, ?, ?, 0)',
                    (key, plan, days, created_by, datetime.now().isoformat(), max_uses)
                )
                keys.append(key)
            conn.commit()
            logger.info(f"{qty} key(s) ({plan}, max_uses={max_uses}) generated by {created_by}")
            return keys
        except Exception as e:
...
def redeem_key_db(key: str, user_id: int):
    """Returns (success: bool, message: str)"""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
            c.execute('SELECT plan, days, max_uses, use_count FROM redeem_keys WHERE key = ?', (key,))
            row = c.fetchone()
            if not row:
                return False, "❌ Invalid key. Please check and try again."
            plan, days, max_uses, use_count = row

            # Check if user already used this key
            c.execute('SELECT 1 FROM key_uses WHERE key = ? AND user_id = ?', (key, user_id))
            if c.fetchone():
                return False, "❌ You have already redeemed this key."

            if use_count >= max_uses:
                return False, "❌ This key has reached its maximum number of uses."

            # Record use
            c.execute('INSERT INTO key_uses (key, user_id, used_at) VALUES (?, ?, ?)',
                      (key, user_id, datetime.now().isoformat()))
            c.execute('UPDATE redeem_keys SET use_count = use_count + 1, used_by = ?, used_at = ? WHERE key = ?',
            ...
            if days >= 36500:
                new_expiry = datetime(9999, 12, 31)
            else:
                new_expiry = start + timedelta(days=days)
            save_subscription(user_id, new_expiry)
            plan_info = KEY_PLANS.get(plan, {})
            label = plan_info.get('label', plan)
            uses_left = max_uses - (use_count + 1)
            if days >= 36500:
                exp_str = "Never (Lifetime)"
            else:
                exp_str = new_expiry.strftime('%Y-%m-%d')
            uses_info = f"♾️ Unlimited uses" if max_uses >= 9999 else f"Uses left on this key: {uses_left}/{max_uses}"
            return True, f"✅ Key redeemed!\nPlan: *{label}*\nExpiry: `{exp_str}`\n{uses_info}"
        except Exception as e:
            logger.error(f"Error redeeming key {key}: {e}", exc_info=True)
            return False, "❌ Error redeeming key. Try again."
        finally:
            conn.close()

def list_keys_db(created_by: int = None, show_used: bool = False):
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    try:
        if created_by:
            c.execute('SELECT key, plan, days, use_count, max_uses, created_at FROM redeem_keys WHERE created_by = ? ORDER BY created_at DESC LIMIT 20', (created_by,))
        else:
            c.execute('SELECT key, plan, days, use_count, max_uses, created_at FROM redeem_keys ORDER BY created_at DESC LIMIT 20')
        return c.fetchall()
    finally:
        conn.close()
...
def create_main_menu_inline(user_id):
    ...
    buttons = [
        ...
        types.InlineKeyboardButton('🤖 MANJIRO AI', callback_data='mpx_ai')
    ]
...
def create_admin_panel():
    ...
    key_status = "✅ Set" if GEMINI_API_KEY and GEMINI_API_KEY != "PUT_YOUR_GEMINI_API_KEY_HERE" else "❌ Not Set"
    markup.row(types.InlineKeyboardButton(f'🔑 Gemini API Key ({key_status})', callback_data='set_gemini_key'))
    markup.row(types.InlineKeyboardButton('🎫 Generate Subscription Key', callback_data='gen_key_menu'))
    markup.row(types.InlineKeyboardButton('📋 List Generated Keys', callback_data='list_keys'))
    markup.row(types.InlineKeyboardButton('🔙 Back to Main', callback_data='back_to_main'))
    return markup

def create_gen_key_menu():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        types.InlineKeyboardButton('1️⃣ 1 Day',    callback_data='genkey_1d'),
        types.InlineKeyboardButton('3️⃣ 3 Days',   callback_data='genkey_3d')
    )
    markup.row(
        types.InlineKeyboardButton('8️⃣ 8 Days',   callback_data='genkey_8d'),
        types.InlineKeyboardButton('🔟 15 Days',  callback_data='genkey_15d')
    )
    markup.row(
        types.InlineKeyboardButton('📅 1 Month',  callback_data='genkey_1month'),
        types.InlineKeyboardButton('📆 3 Months', callback_data='genkey_3months')
    )
    markup.row(types.InlineKeyboardButton('♾️ Lifetime', callback_data='genkey_lifetime'))
    markup.row(types.InlineKeyboardButton('🔙 Back to Admin Panel', callback_data='admin_panel'))
    return markup

def create_qty_menu(plan: str):
    markup = types.InlineKeyboardMarkup(row_width=4)
    markup.row(
        types.InlineKeyboardButton('1',  callback_data=f'gkqty_{plan}_1'),
        types.InlineKeyboardButton('3',  callback_data=f'gkqty_{plan}_3'),
        types.InlineKeyboardButton('5',  callback_data=f'gkqty_{plan}_5'),
        types.InlineKeyboardButton('10', callback_data=f'gkqty_{plan}_10'),
    )
    markup.row(
        types.InlineKeyboardButton('20', callback_data=f'gkqty_{plan}_20'),
        types.InlineKeyboardButton('50', callback_data=f'gkqty_{plan}_50'),
    )
    markup.row(types.InlineKeyboardButton('🔙 Back', callback_data='gen_key_menu'))
    return markup

def create_maxuse_menu(plan: str, qty: int):
    markup = types.InlineKeyboardMarkup(row_width=3)
    markup.row(
        types.InlineKeyboardButton('1 user',  callback_data=f'gkuse_{plan}_{qty}_1'),
        types.InlineKeyboardButton('3 users', callback_data=f'gkuse_{plan}_{qty}_3'),
        types.InlineKeyboardButton('5 users', callback_data=f'gkuse_{plan}_{qty}_5'),
    )
    markup.row(
        types.InlineKeyboardButton('10 users',       callback_data=f'gkuse_{plan}_{qty}_10'),
        types.InlineKeyboardButton('50 users',       callback_data=f'gkuse_{plan}_{qty}_50'),
        types.InlineKeyboardButton('♾️ Unlimited',   callback_data=f'gkuse_{plan}_{qty}_9999'),
    )
    markup.row(types.InlineKeyboardButton('🔙 Back', callback_data=f'gen_key_menu'))
    return markup
...
def handle_txt_file(file_path, script_owner_id, user_folder, file_name, message):
    try:
        save_user_file(script_owner_id, file_name, 'txt')
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
        preview = content[:3000] if len(content) > 3000 else content
        truncated_note = "\n\n_(Showing first 3000 chars — file saved.)_" if len(content) > 3000 else ""
        bot.reply_to(message,
            f"📄 *{file_name}* saved!\n\n```\n{preview}\n```{truncated_note}\n\n✅ Thank you for using my bot!",
            parse_mode='Markdown')
    except Exception as e:
        logger.error(f"Error processing txt file {file_name} for {script_owner_id}: {e}", exc_info=True)
        bot.reply_to(message, f"Error processing text file: {str(e)}")
...
def _logic_send_welcome(message):
    ...
    welcome_msg_text = (f"Welcome, {user_name}!\n\nYour User ID: `{user_id}`\n"
                        ...
                        f"Upload single scripts, `.zip` archives, or `.txt` files.\n\n"
                        f"Use 💬 *AI Chat* button for free AI conversation!\n"
...
def _logic_upload_file(message):
    ...
    if current_files >= file_limit:
        ...
        return
    bot.reply_to(message, "Send your Python (`.py`), JS (`.js`), ZIP (`.zip`), or text (`.txt`) file.")
...
def handle_mpx_command(message):
    ...
    if GEMINI_API_KEY == "PUT_YOUR_GEMINI_API_KEY_HERE" or not GEMINI_API_KEY:
        bot.reply_to(message, "⚠️ Gemini API key not set. Ask the admin to configure it via the Admin Panel.")
        return
    ...
    try:
        ...
        response = requests.post(get_gemini_url(), headers=headers, json=payload, timeout=30)
        response.raise_for_status()

        result = response.json()
        candidates = result.get('candidates', [])
        if not candidates:
            bot.reply_to(message, "No response from Gemini. Try again.")
            return

        answer = candidates[0].get('content', {}).get('parts', [{}])[0].get('text', 'No response.')
        ...
        bot.reply_to(message, "Error connecting to Gemini API. Check your API key or try again later.")
    except Exception as e:
        logger.error(f"Error in /mpx command: {e}", exc_info=True)
        bot.reply_to(message, f"An error occurred: {str(e)}")

@bot.message_handler(commands=['aichat'])
def handle_aichat_command(message):
    user_id = message.from_user.id
    if bot_locked and user_id not in admin_ids:
        bot.reply_to(message, "Bot is currently locked. Try again later.")
        return
    if GEMINI_API_KEY == "PUT_YOUR_GEMINI_API_KEY_HERE" or not GEMINI_API_KEY:
        bot.reply_to(message, "⚠️ Gemini API key not set. Ask the admin to configure it.")
        return
    ai_chat_users.add(user_id)
    bot.reply_to(message,
        "🤖 *MANJIRO AI Chat Mode ON!*\n\n"
        "You can now chat with AI freely — just send any message!\n"
        "Type /exitchat to exit AI chat mode.\n\n"
        "_Thank you for using my bot!_ 🙏",
        parse_mode='Markdown')

@bot.message_handler(commands=['exitchat'])
def handle_exitchat_command(message):
    user_id = message.from_user.id
    if user_id in ai_chat_users:
        ...
        bot.reply_to(message, "You are not in AI chat mode. Use /aichat to start.")

def _send_gemini_reply(message, query):
    """Helper: send a query to Gemini and reply."""
    bot.send_chat_action(message.chat.id, 'typing')
    try:
        headers = {"Content-Type": "application/json"}
        payload = {
        ...
        candidates = result.get('candidates', [])
        if not candidates:
            bot.reply_to(message, "No response from AI. Try again.")
            return
        answer = candidates[0].get('content', {}).get('parts', [{}])[0].get('text', 'No response.')
        if len(answer) > 4000:
            for x in range(0, len(answer), 4000):
        ...
        logger.error(f"Gemini API error in aichat: {e}")
        bot.reply_to(message, "Error connecting to AI. Try again later.")
    except Exception as e:
        logger.error(f"Error in aichat: {e}", exc_info=True)
        bot.reply_to(message, f"An error occurred: {str(e)}")
...
def handle_redeem_command(message):
    ...
    if len(parts) < 2:
        bot.reply_to(message, "Usage: `/redeem YOUR-KEY-HERE`\nExample: `/redeem MJR-ABCDE-12345-FGHIJ-67890`", parse_mode='Markdown')
        return
    key = parts[1].strip().upper()
    success, msg = redeem_key_db(key, user_id)
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "🤖 MANJIRO AI": lambda m: handle_mpx_command(m),
    "💬 AI Chat": lambda m: handle_aichat_command(m),
    "🚪 Exit AI Chat": lambda m: handle_exitchat_command(m),
}
...
@bot.message_handler(func=lambda message: message.text and re.match(r'^MJR-[A-Z0-9]{5}-[A-Z0-9]{5}-[A-Z0-9]{5}-[A-Z0-9]{5}$', message.text.strip().upper()))
def handle_plain_key_message(message):
    """Auto-redeem if user pastes a key directly as a message (no /redeem command)."""
    user_id = message.from_user.id
    ...
    bot.reply_to(message, msg, parse_mode='Markdown')
    if success:
        logger.info(f"User {user_id} auto-redeemed key {key} via plain message.")
        try:
            bot.delete_message(message.chat.id, message.message_id)
        except Exception:
            pass

@bot.message_handler(func=lambda message: message.from_user.id in ai_chat_users and message.content_type == 'text')
def handle_aichat_message(message):
    user_id = message.from_user.id
    if bot_locked and user_id not in admin_ids:
        bot.reply_to(message, "Bot is locked. AI chat unavailable.")
        return
    if message.text and message.text.startswith('/'):
        return  # Let command handlers take over
    _send_gemini_reply(message, message.text)
...
def handle_file_upload_doc(message):
    ...
    if file_ext not in ['.py', '.js', '.zip', '.txt']:
        bot.reply_to(message, "Unsupported type! Only `.py`, `.js`, `.zip`, `.txt` allowed.")
        return
    max_file_size = 200 * 1024 * 1024  # 200MB upload limit
    ...
    try:
        ...
        if file_ext == '.zip':
            ...
            elif file_ext == '.txt': handle_txt_file(file_path, user_id, user_folder, file_name, message)
...
def handle_callbacks(call):
    ...
    try:
        if data == 'upload':
        ...
        elif data == 'set_gemini_key':
            owner_required_callback(call, set_gemini_key_callback)
        elif data == 'gen_key_menu':
            admin_required_callback(call, gen_key_menu_callback)
        elif data.startswith('genkey_'):
            admin_required_callback(call, lambda c: handle_genkey_plan(c))
        elif data.startswith('gkqty_'):
            admin_required_callback(call, lambda c: handle_genkey_qty(c))
        elif data.startswith('gkuse_'):
            admin_required_callback(call, lambda c: handle_genkey_maxuse(c))
        elif data == 'list_keys':
            admin_required_callback(call, list_keys_callback)
            ...
            bot.send_message(call.message.chat.id,
                "🤖 *MANJIRO AI*\n\n"
                "• `/mpx <question>` — Ask a single question\n"
                "• `/aichat` — Enter free AI chat mode (just type naturally!)\n"
                "• `/exitchat` — Exit AI chat mode\n\n"
                "_Thank you for using my bot!_ 🙏",
                parse_mode='Markdown')
        elif data == 'uptime':
            bot.answer_callback_query(call.id)
...
def gen_key_menu_callback(call):
    bot.answer_callback_query(call.id)
    try:
        bot.edit_message_text(
            "🎫 *Generate Subscription Key*\n\nStep 1: Select a plan:",
            call.message.chat.id, call.message.message_id,
            reply_markup=create_gen_key_menu(), parse_mode='Markdown'
        )
    except Exception as e:
        logger.error(f"Error showing gen key menu: {e}")

def handle_genkey_plan(call):
    plan = call.data.replace('genkey_', '')
    plan_info = KEY_PLANS.get(plan)
    if not plan_info:
        bot.answer_callback_query(call.id, "Unknown plan.", show_alert=True)
        return
    bot.answer_callback_query(call.id)
    try:
        bot.edit_message_text(
            f"🎫 *Generate Subscription Key*\n\nPlan: *{plan_info['label']}*\n\nStep 2: How many keys to generate?",
            call.message.chat.id, call.message.message_id,
            reply_markup=create_qty_menu(plan), parse_mode='Markdown'
        )
    except Exception as e:
        logger.error(f"Error showing qty menu: {e}")

def handle_genkey_qty(call):
    # data format: gkqty_{plan}_{qty}
    parts = call.data.split('_', 2)  # ['gkqty', plan, qty]  — but plan may have no underscore
    try:
        _, plan, qty_str = call.data.split('_', 2)
        qty = int(qty_str)
        plan_info = KEY_PLANS.get(plan)
        if not plan_info:
            bot.answer_callback_query(call.id, "Unknown plan.", show_alert=True)
        ...
        bot.answer_callback_query(call.id)
        bot.edit_message_text(
            f"🎫 *Generate Subscription Key*\n\nPlan: *{plan_info['label']}*\nQuantity: *{qty} key{'s' if qty > 1 else ''}*\n\nStep 3: How many users can redeem each key?",
            call.message.chat.id, call.message.message_id,
            reply_markup=create_maxuse_menu(plan, qty), parse_mode='Markdown'
        )
    except Exception as e:
        logger.error(f"Error in handle_genkey_qty: {e}", exc_info=True)
        bot.answer_callback_query(call.id, "Error. Try again.", show_alert=True)

def handle_genkey_maxuse(call):
    # data format: gkuse_{plan}_{qty}_{max_uses}
    try:
        _, plan, qty_str, max_uses_str = call.data.split('_', 3)
        qty = int(qty_str)
        ...
        if not plan_info:
            bot.answer_callback_query(call.id, "Unknown plan.", show_alert=True)
            return
        bot.answer_callback_query(call.id, f"Generating {qty} key(s)...")
        keys = generate_key(plan, call.from_user.id, qty=qty, max_uses=max_uses)
        if not keys:
            bot.send_message(call.message.chat.id, "❌ Failed to generate keys. Try again.")
            return
        max_uses_label = "♾️ Unlimited" if max_uses >= 9999 else str(max_uses)
        lines = [
            f"🎫 *{len(keys)} Key{'s' if len(keys) > 1 else ''} Generated!*\n",
            f"Plan: *{plan_info['label']}*",
            f"Max users per key: *{max_uses_label}*\n",
        ]
        ...
        if len(text) > 4000:
            text = text[:4000] + "\n...(truncated)"
        markup = types.InlineKeyboardMarkup()
        markup.row(types.InlineKeyboardButton('🎫 Generate More', callback_data='gen_key_menu'))
        markup.row(types.InlineKeyboardButton('🔙 Admin Panel', callback_data='admin_panel'))
        bot.send_message(call.message.chat.id, text, parse_mode='Markdown', reply_markup=markup)
    except Exception as e:
        logger.error(f"Error in handle_genkey_maxuse: {e}", exc_info=True)
        bot.send_message(call.message.chat.id, "❌ Error generating keys. Try again.")

def list_keys_callback(call):
    bot.answer_callback_query(call.id)
    rows = list_keys_db(created_by=call.from_user.id if call.from_user.id != OWNER_ID else None)
    if not rows:
        bot.send_message(call.message.chat.id, "No keys generated yet.")
        return
    lines = ["📋 *Generated Keys (last 20):*\n"]
    for key, plan, days, use_count, max_uses, created_at in rows:
        plan_info = KEY_PLANS.get(plan, {})
        label = plan_info.get('label', plan)
        uses_label = "♾️" if max_uses >= 9999 else str(max_uses)
        if use_count >= max_uses:
            status = "🔴 Fully Used"
        elif use_count > 0:
            status = f"🟡 {use_count}/{uses_label} used"
        else:
            status = f"🟢 Unused (0/{uses_label})"
        created_short = created_at[:10] if created_at else "?"
        lines.append(f"`{key}`\n  {label} | {status} | {created_short}\n")
    text = "\n".join(lines)
    if len(text) > 4000:
        text = text[:4000] + "\n...(truncated)"
    markup = types.InlineKeyboardMarkup()
...
def set_gemini_key_callback(call):
    bot.answer_callback_query(call.id)
    current = "✅ Currently set" if GEMINI_API_KEY and GEMINI_API_KEY != "PUT_YOUR_GEMINI_API_KEY_HERE" else "❌ Not set yet"
    msg = bot.send_message(call.message.chat.id,
        f"🔑 *Gemini API Key*\nStatus: {current}\n\nSend your new Gemini API key now.\nGet one free at: https://aistudio.google.com\n\n/cancel to abort.",
        parse_mode='Markdown')
    bot.register_next_step_handler(msg, process_set_gemini_key)

def process_set_gemini_key(message):
    ...
    if not new_key or len(new_key) < 20:
        bot.reply_to(message, "Invalid key. Please send a valid Gemini API key or /cancel.")
        msg = bot.send_message(message.chat.id, "Send your Gemini API key or /cancel.")
        bot.register_next_step_handler(msg, process_set_gemini_key)
        return
    ...
    try:
        ...
        pass
    bot.send_message(message.chat.id, "✅ Gemini API key updated successfully!\n\nThe key message was deleted for security. Test it with `/mpx Hello`", parse_mode='Markdown')
    logger.info(f"Gemini API key updated by Owner {message.from_user.id}")
...
@bot.message_handler(commands=['ram','usage','resources'])
def ram_usage_cmd(message):
    try:
...
def owner_dashboard(message):
    ...
    try:
        ...
        running = 0
        for _, info in list(bot_scripts.items()):
            try:
        ...
        bot.reply_to(
            ...
            f"Free: {round(mem.available/1024**3,2)} GB\\n\\n"
            f"⚙️ CPU: {cpu}%\\n\\n"
            f"💾 Disk: {disk.percent}%\\n"
            f"Free Disk: {round(disk.free/1024**3,2)} GB\\n\\n"
        ...
        bot.reply_to(message, f"Dashboard error: {e}")
