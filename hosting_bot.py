...
def install_requirements(packages: str, upgrade: bool = True) -> tuple[bool, str]:
    """
    Install pip packages. `packages` is a space- or newline-separated string.
    If upgrade=True, passes --upgrade to pip. Returns (success, output_text).
    """
    ...
    try:
        ...
        return False, "❌ pip install timed out (120 s). Try fewer packages."
...
async def handle_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in WAITING_INSTALL:
        ...
        pkg_list_display = packages.replace("\n", "  ")
        mode_label = "⬆️ Upgraded" if upgrade else "📌 Normal (no upgrade)"
        thinking = await update.message.reply_text(
            f"📦 Installing ({mode_label}):\n<code>{pkg_list_display}</code>\n\n⏳ This may take a moment…",
            parse_mode=ParseMode.HTML,
        )
